...
# 「図にして」は裁定 U で足した。それまで「全体像を図にして」は疑問形では
# なかったので地図動詞を見るまでもなく map だったが、「〜図にして**もらえますか**」
# が疑問形として拾えるようになった以上、生成動詞の一覧に無いと丁寧な地図依頼が
# QA へ落ちる。「描いて」「作って」と同種の生成動詞なので歯止め側に入れる。
MAP_VERB_CUES = ["整理して", "作成", "作って", "描いて", "生成",
                 "地図として", "地図に", "マップ化", "図にして"]
# 疑問形の合図。末尾記号は endswith、語は部分一致で見る。
QUESTION_MARKS = ("?", "？")
# 裁定 U: 「ますか」「ましたか」「でしょうか」を足す。「ですか」だけでは
# 丁寧体の問いを取りこぼす — 「どうなっていますか」「何が分かりましたか」
# 「〜でしょうか」はどれも「ですか」を部分文字列に含まないため。
# 3 条件ガード (QA cue + 疑問形 + 地図動詞なし) は変えていないので、
# 地図語を含まない文・地図生成の動詞を伴う文の判定は動かない。
QUESTION_CUES = ["教えて", "ですか", "ますか", "ましたか", "でしょうか"]
# 詳細度の指定
LEVEL_CUES = {
