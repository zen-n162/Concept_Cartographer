...
def verify_scene_offline(plan: dict[str, Any], scene: dict[str, Any]) -> dict[str, Any]:
    """MCP を介さずに、生成済み .excalidraw シーンと計画を突合する。

    ACA 到達前の fallback 経路 (target=file) でも独立検証を成立させるため
    (実運用計画 §3-2)。ライブキャンバスが無いだけで、照合内容は同じ。
    """
    ...
    expected_ids: list[str] = []
    for island in plan.get("islands", []):
        expected_ids += [island_element_id(island["community_id"]),
                         island_label_id(island["community_id"])]
    for node in plan["nodes"]:
        expected_ids.append(node_element_id(node["id"]))
    for edge in plan.get("edges", []):
    ...
    for island in plan.get("islands", []):
        if island.get("is_gap"):
            el = by_id.get(island_element_id(island["community_id"]))
            if el is not None and not (
                el.get("strokeStyle") == "dashed"
                and (el.get("opacity", 100) or 100) < 100
            ):
                gap_style_violations.append(island_element_id(island["community_id"]))

    passed = not missing and not label_mismatches and not gap_style_violations
    logger.info("verify(offline) passed=%s expected=%d scene=%d",
                passed, len(expected_ids), len(elements))
    return {
        "passed": passed,
        "expected_element_count": len(expected_ids),
        "canvas_element_count": len(elements),
        ...
        "label_mismatches": label_mismatches,
        "gap_style_violations": gap_style_violations,
        "describe_scene": f"offline scene: {len(elements)} elements",
    }
